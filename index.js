...
function buildPanelRow(disabled = []) {
    return new ActionRowBuilder().addComponents(
        PANEL_BUTTONS.map(b => new ButtonBuilder()
            .setCustomId(b.id).setLabel(b.label).setStyle(b.style)
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        // ── /skinisveopomene (skida sve opomene jednom korisniku)
        if (commandName === 'skinisveopomene') {
            const target = interaction.options.getUser('korisnik');
            ...
            if (ch) await ch.send({ embeds: [new EmbedBuilder().setColor(0x2ECC71).setTitle('❌ Sve opomene skinute')
                .addFields(
                    { name: 'Korisnik',  value: `${target}`, inline: true },
                    { name: 'Moderator', value: `${mod}`,    inline: true },
        ...
        if (commandName === 'zatvori') {
            ...
            return interaction.editReply(`✅ Dugme **${tip}** je onemogućeno.`);
        }

        // ── /otvori (ponovo aktiviraj dugme)
        if (commandName === 'otvori') {
            ...
            savePanelState();
            await updatePanelMessage();
            return interaction.editReply(`✅ Dugme **${tip}** je ponovo aktivirano.`);
        }

        // ── /skinisvima ──────────────────────────────────────────────────
        if (commandName === 'skinisvima') {
            const role = interaction.options.getRole('rola');
            await interaction.editReply(`⏳ Skidčam rolu **${role.name}** svim članovima, pričekaj...`);
            const members = await guild.members.fetch();
            const targets = members.filter(m => !m.user.bot && m.roles.cache.has(role.id));
            await Promise.allSettled(targets.map(m => m.roles.remove(role).catch(() => {})));
            return interaction.editReply(`✅ Rola **${role.name}** skinuta svim članovima (${targets.size} korisnika).`);
        }
