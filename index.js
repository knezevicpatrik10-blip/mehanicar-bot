...
const AFK_VOICE_CHANNEL = '1485998086395396136'; // AFK voice kanal
const AFK_TIMEOUT_MS    = 10 * 60 * 1000;        
const VC_STATS_FILE     = './vcStats.json';
const vcStartTimes      = new Map();
const afkTimers         = new Map();
let   vcStats           = {};

if (fs.existsSync(VC_STATS_FILE)) {
    try { vcStats = JSON.parse(fs.readFileSync(VC_STATS_FILE, 'utf8')); }
    catch { vcStats = {}; }
}
function saveVcStats() {
    fs.writeFileSync(VC_STATS_FILE, JSON.stringify(vcStats, null, 2));
}
...
// ─── Blacklista ──────────────────────────────────────────────────────────
let blacklist = {}; let blState = { messageId: null };
if (fs.existsSync('./blacklist.json'))    { try { blacklist = JSON.parse(fs.readFileSync('./blacklist.json','utf8')); } catch {} }
if (fs.existsSync('./blacklistState.json')) { try { blState  = JSON.parse(fs.readFileSync('./blacklistState.json','utf8')); } catch {} }
function saveBlacklist() { fs.writeFileSync('./blacklist.json',      JSON.stringify(blacklist, null, 2)); }
function saveBlState()   { fs.writeFileSync('./blacklistState.json', JSON.stringify(blState,   null, 2)); }
...
// ─── Tablica opomena ──────────────────────────────────────────────────────────
const TABLE_FILE = './tableState.json';
let   tableState = { messageId: null };

if (fs.existsSync(TABLE_FILE)) {
    try { tableState = JSON.parse(fs.readFileSync(TABLE_FILE, 'utf8')); }
    catch { tableState = { messageId: null }; }
}
function saveTableState() {
    fs.writeFileSync(TABLE_FILE, JSON.stringify(tableState, null, 2));
}
...
client.on('clientReady', () => {
    ...
    client.user.setActivity('MEHANICARI ON TOP <3', { type: 3 }); // WATCHING
    scheduleDailyReset();
});
